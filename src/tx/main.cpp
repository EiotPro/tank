...
void loop() {
  ...
  if (millis() - lastTransmission >= TRANSMISSION_INTERVAL) {
    ...
    Serial.println("\n========================================");
    Serial.println("📤 TRANSMITTING OPTIMIZED DATA");
    Serial.println("========================================");
    ...
    // 🚀 OPTIMIZATION: Send ONLY raw sensor value (2 bytes: 0-1023)
    // Receiver will do all calculations - much more efficient!
    int rawSensorValue = analogRead(WATER_SENSOR_PIN);
    String hexPayload = decimalToHex(rawSensorValue);

    Serial.print("🔢 OPTIMIZED: Sending raw value (");
    Serial.print(rawSensorValue);
    Serial.print(") as HEX: ");
    Serial.println(hexPayload);
    Serial.println("📡 Efficient LoRa transmission...");

    // Send data via LoRa
    if (sendLoRaData(hexPayload)) {
      Serial.println("✅ Optimized transmission successful!");
      Serial.println("🚀 Minimal data sent - maximum efficiency!");
    } else {
      Serial.println("❌ LoRa transmission failed!");
